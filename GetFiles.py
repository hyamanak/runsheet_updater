...
class GetFiles():
    def __init__(self):
        ...
        self.__sheet_name = "daily_source_updates_ja_"
        ...
        self.en_dfile = self.__date + "_en" + self.__extension
        self.jp_dfile = self.__date + "_jp" + self.__extension

        self.__daily_en = self.__base_url + self.__year + "/" + self.__month + "/" + self.__sheet_name + self.__date + self.__extension
    ...
    def download_csv_from_jp_runsheet(self):
        ...
        with open(self.jp_dfile, 'w+', encoding='utf-8') as jp_file:
            jp_file.write(jp_data)

    def download_csv_from_gitlab(self):
        response = requests.get(self.__daily_en, verify=False)
        response.raise_for_status()

        with open(self.en_dfile, 'w+') as en_file:
